pragma solidity ^0.4.2;


import "./sysbase/OwnerNamed.sol";
import "./sysbase/BaseModule.sol";


contract MonitorModuleManager is BaseModule {

    using LibString for *;
    using LibInt for *;

    string moduleName;

    string moduleVersion;

    string contractName;

    string contractVersion;

    string sysModuleId;

    uint reversion;

    // contractName => contractId ,eg : UserMananger => contract001
    mapping (string => string) innerContractMapping;

    enum MODULE_ERROR {
    NO_ERROR
    }

    enum ROLE_DEFINE {
    ROLE_SUPER,
    ROLE_ADMIN,
    ROLE_PLAIN
    }

    event Notify(uint _error, string _info);

    function MonitorModuleManager(){
        reversion = 0;
        register("MonitorModuleManager", "0.0.1.0");

        moduleName = "MonitorModuleManager";
        //设置模块名称【根据实际修改】
        moduleVersion = "0.0.1.0";
        //设置模块版本号
        sysModuleId = "monitorModuleId001";
        //显示指定模块ID

        string memory _json = "{";
        _json = _json.concat("\"moduleId\":\"", sysModuleId, "\",");
        _json = _json.concat("\"moduleName\":\"", moduleName, "\",");
        _json = _json.concat("\"moduleVersion\":\"", moduleVersion, "\",");
        _json = _json.concat("\"icon\":\"data:image/png;base64,\"", ",");
        _json = _json.concat("\"moduleEnable\":", "0", ",");
        //模块开关：0-放权；1-控权【需要修改】
        _json = _json.concat("\"moduleType\":", "1", ",");
        // 模块类型 1 系统模块
        _json = _json.concat("\"moduleUrl\":\"http://192.168.9.18/dapp-monitor/#\"", ",");
        _json = _json.concat("\"moduleText\":\"监控DAPP\"", ",");
        _json = _json.concat("\"moduleDescription\":\"", moduleName.concat(moduleVersion, "-监控DAPP模块"), "\"");
        _json = _json.concat("}");

        uint isExists = moduleIsExist(sysModuleId);
        uint ret = 0;

        if (isExists != 1) {
            log("publish monitor module , exec update ...");
            ret = updModule(_json);
            sysModuleId = ret.recoveryToString();
        }
        else {
            log("publish monitor module , exec insert ...");
            ret = addModule(_json);
            sysModuleId = ret.recoveryToString();
        }

        /* uint ret = addModule(_json); 
        sysModuleId = ret.recoveryToString(); */
        log("construct sysModuleId:", sysModuleId);

    }

}
